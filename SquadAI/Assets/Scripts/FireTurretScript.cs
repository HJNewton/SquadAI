...
public class FireTurretScript : MonoBehaviour
{
    ...
    void SelectTarget()
    {
        Collider[] hitColliders = Physics.OverlapSphere(transform.position, turretRange); // Get all colliders in a certain radius
        ...
        if (target)
        {
            if (Vector3.Distance(transform.position, target.position) > turretRange)
            {
    ...
    void Fire()
    {
        ...
        Instantiate(fireballPrefab, firePoint.position, Quaternion.LookRotation(lookRotation));
    }
